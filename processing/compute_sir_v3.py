...
def sir(channels: np.ndarray, M, K, precoder):
    ...
    for k in range(K):
        ...
        if np.all((v_k == 0)):
            ...
            interfence = np.sum([np.abs(v_k_H @ h_i) ** 2 for h_i in h_others]) + np.linalg.norm(v_k)**2
            sir_k = np.abs(v_k_H @ h_k) ** 2 / interfence
        sir[k] = sir_k
...
if __name__ == '__main__':
    ...
    res = {
        "ULA": np.zeros(shape=(len(precoders), len(num_nodes), num_simulations)).tolist(),
        "URA": np.zeros(shape=(len(precoders), len(num_nodes), num_simulations)).tolist(),
        "iid": np.zeros(shape=(len(precoders), len(num_nodes),  num_simulations)).tolist(),
    }
    ...
    for conf, prec_values in res.items():
        for precoder, k_values in enumerate(prec_values):
            avg_c = np.zeros_like(num_nodes)
            for k, values in enumerate(k_values):
                avg_c[k] = 10 * np.log10(np.nanmedian(values))
            linestyle = "-"
            if conf == "ULA":
                linestyle = "--"
            elif conf == "iid":
                linestyle = "dotted"
            ax.plot(num_nodes, avg_c, label=conf + " - " + precoders[precoder] + " - " + str(num_nodes[k]),
                    linewidth=1,
